# androidmcp — development & engineering notes

The public overview, capability list, and quick start are in [README.md](../README.md).
This document is the deeper reference: how to build from source, the full design notes and
caveats, and the complete v1/v2/v3 roadmap — including the things that were deliberately
**not** done, and why.

## Building from source

### A toolchain known to build this
...
gitignored). On Windows use `gradlew.bat`; `scripts/build-and-install.sh` needs bash.

### Signing — read this before `adb install`

Unless a real key is configured, a release build is signed with the building machine's
`~/.android/debug.keystore`, so its certificate differs from machine to machine. Installing over an
app signed by a *different* debug key fails with `INSTALL_FAILED_UPDATE_INCOMPATIBLE`. Check the
APK's certificate first (`apksigner` lives in `$ANDROID_HOME/build-tools/<version>/`):

```bash
apksigner verify --print-certs app/build/outputs/apk/release/app-release.apk | grep SHA-256
```

If it differs from the copy already on the device, pick one:

- **Uninstall first:** `adb uninstall com.sixoffive.androidmcp` — this wipes tokens, folder grants,
  capability toggles and special-access approvals.
- **Use one real key everywhere:** set `ANDROIDMCP_KEYSTORE`, `ANDROIDMCP_KEYSTORE_PASSWORD`,
  `ANDROIDMCP_KEY_ALIAS` and `ANDROIDMCP_KEY_PASSWORD` (see [../app/build.gradle.kts](../app/build.gradle.kts)).
  The right long-term fix; the first install with a new key still needs the one-time uninstall.

### Build, install, verify

```bash
./gradlew :app:testDebugUnitTest          # JVM tests; no device or emulator needed
./gradlew :app:assembleRelease            # use release for anything left running (not debuggable)
...
1. Allow notifications when asked — the per-call approval prompt needs it.
2. Enable only the capabilities you want; each card states exactly what it exposes and the risk.
...
5. Press **Generate token**, then take it with **Copy connect command** (or scan the QR) rather than
   reading it off the screen — tokens mix `0`/`O` and `l`/`I`, and a misread token just returns 401.

### Things that look like bugs but aren't

| symptom | actual cause |
|---|---|
| an MCP client reports `SSE stream ended without a response` | the `adb forward` dropped mid-stream; the server is fine. Check `adb forward --list`, then forward again. |
...
| Claude Desktop quietly drops the server | its config file accepts only **stdio** entries; use the `mcp-remote` bridge (see Connecting a client). |
...
```bash
# app shows e.g. "listening on 192.168.1.50:8765"
claude mcp add --transport http tablet http://192.168.1.50:8765/mcp --header "Authorization: Bearer <TOKEN>"
```

**Tailscale (bind: tailscale) — from anywhere, WireGuard-encrypted (recommended):**
```bash
# app shows the device's 100.x tailnet address
claude mcp add --transport http tablet http://100.64.0.3:8765/mcp --header "Authorization: Bearer <TOKEN>"
```
...
- [x] **Settings UI grouped** — the one long Server page is split into **Server** /
      **Connection & transport** / **Startup** cards. Layout verified on-device via
      ...
      (11 categories, per-group counts, expand/collapse) works on the phone. Screenshots: [screenshots/](screenshots/).
